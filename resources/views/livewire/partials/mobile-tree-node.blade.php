...
@if ($shouldShow)
    <div class="flex flex-col items-center relative group">
        {{-- Connector Point Top --}}
        <div class="w-2 h-2 bg-white border-2 border-gray-400 rounded-full absolute -top-1 z-20 group-hover:border-primary-500 group-hover:scale-125 transition-all"></div>
        ...
        <div class="relative">
            ...
            <div class="w-28 {{ $cardBg}} {{ $borderWidth }} {{ $borderColor }} {{ $ringClass }} {{ $topBorder }} rounded-lg p-2 text-center {{ $shadowClass }} {{ $statusClass }} transition-all hover:scale-105 active:shadow-xl cursor-pointer relative"
                ...
                @if ($generationLevel == 1)
                    <div class="absolute -top-0.5 -left-0.5 w-3 h-3 border-t-2 border-l-2 border-yellow-500 rounded-tl-md"></div>
                    <div class="absolute -top-0.5 -right-0.5 w-3 h-3 border-t-2 border-r-2 border-yellow-500 rounded-tr-md"></div>
                    <div class="absolute -bottom-0.5 -left-0.5 w-3 h-3 border-b-2 border-l-2 border-yellow-500 rounded-bl-md"></div>
                    <div class="absolute -bottom-0.5 -right-0.5 w-3 h-3 border-b-2 border-r-2 border-yellow-500 rounded-br-md"></div>
                @endif
                ...
                {{-- Avatar --}}
                <div class="w-12 h-12 mx-auto mb-1 rounded-full border-2 {{ $avatarBorder }} shadow-md overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 group-hover:scale-105 transition-transform">
                ...
                {{-- Name --}}
                <p class="{{ $nameClass }} text-xs truncate leading-tight" style="text-shadow: 0 1px 2px rgba(255,255,255,0.8);">
                    {{ $person->name }}
                </p>
                ...
                @if ($person->generation_id)
                    <span class="inline-block mt-1 px-1.5 py-0.5 bg-indigo-500/90 text-white text-[8px] font-bold rounded leading-none">
                        Đời {{ $person->generation_id }}
                    </span>
                @endif
                
                {{-- Hover Action Buttons --}}
                <div class="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-30">
                    <button class="bg-indigo-500 text-white rounded-full p-0.5 shadow-md hover:bg-indigo-600 hover:scale-110 transition-all"
                            title="Tập trung"
                    ...
                    <button class="bg-primary-500 text-white rounded-full p-0.5 shadow-md hover:bg-primary-600 hover:scale-110 transition-all"
                            title="Thêm"
            ...
            @if (($filters['showSpouses'] ?? true) && $spouses && $spouses->count() > 0)
                @foreach ($spouses as $marriage)
                    ...
                    <div class="mt-2 w-24 {{ $spouseBg }} border border-gray-300 {{ $spouseBorder }} border-t-[3px] rounded-md p-1.5 text-center shadow-sm {{ !$spouse->is_alive ? 'grayscale-[20%] opacity-95' : '' }} cursor-pointer hover:bg-opacity-80 transition-all"
                        ...
                        <div class="w-8 h-8 mx-auto mb-1 rounded-full border border-gray-200 shadow-sm overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
